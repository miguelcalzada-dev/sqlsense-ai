...
const STARTER = "-- Escribe tu consulta SQL aquí";
...
export const CHALLENGES: Challenge[] = [
  ...
  },
  {
    ...
    difficulty: "Principiante",
    prompt: "Devuelve el modelo y el precio de todos los coches, ordenados por precio descendente.",
...

  {
    ...
    prompt: "Muestra el modelo y el color de todos los coches de la marca 'BMW'. Necesitarás un JOIN con marcas.",
    hint: "JOIN marcas ON coches.marca_id = marcas.id WHERE marcas.nombre = 'BMW'.",
    starterCode: STARTER,
    canonicalSQL: "SELECT c.modelo, c.color FROM coches c JOIN marcas m ON c.marca_id = m.id WHERE m.nombre = 'BMW';",
  ...
  },
  {
    ...
    starterCode: STARTER,
    canonicalSQL: "SELECT STRFTIME('%Y', fecha) AS anio, COUNT(*) AS total FROM ventas GROUP BY anio ORDER BY anio;",
  ...
  },
  {
    ...
    starterCode: STARTER,
    canonicalSQL: "SELECT m.nombre AS marca, AVG(c.precio) AS media FROM marcas m JOIN coches c ON c.marca_id = m.id GROUP BY m.nombre ORDER BY media DESC;",
  ...
  },
  {
    ...
    difficulty: "Intermedio",
    prompt: "Devuelve 20 ventas mostrando modelo, nombre del cliente, fecha y precio de venta, ordenadas por fecha descendente.",
    hint: "Necesitas tres JOIN: ventas → coches y ventas → clientes.",
    starterCode: STARTER,
    canonicalSQL: "SELECT c.modelo, cl.nombre AS cliente, v.fecha, v.precio_venta FROM ventas v JOIN coches c ON v.coche_id = c.id JOIN clientes cl ON v.cliente_id = cl.id ORDER BY v.fecha DESC LIMIT 20;",
  ...
  },
  {
    ...
    starterCode: STARTER,
    canonicalSQL: "SELECT * FROM coches WHERE precio BETWEEN 25000 AND 60000 ORDER BY precio;",
...

  {
    ...
    difficulty: "Avanzado",
    prompt: "Lista las marcas que hayan realizado más de 5 ventas, con su número de ventas. Ordena de mayor a menor.",
    hint: "GROUP BY + HAVING COUNT(*) > 5.",
    starterCode: STARTER,
    canonicalSQL: "SELECT m.nombre AS marca, COUNT(v.id) AS ventas FROM marcas m JOIN coches c ON c.marca_id = m.id JOIN ventas v ON v.coche_id = c.id GROUP BY m.nombre HAVING COUNT(v.id) > 5 ORDER BY ventas DESC;",
  ...
  },
  {
    ...
    starterCode: STARTER,
    canonicalSQL: "SELECT c.id, c.nombre FROM clientes c LEFT JOIN ventas v ON v.cliente_id = c.id WHERE v.id IS NULL;",
  ...
  },
  {
    ...
    starterCode: STARTER,
    canonicalSQL: "SELECT c.id, c.modelo FROM coches c LEFT JOIN ventas v ON v.coche_id = c.id WHERE v.id IS NULL;",
  ...
  },
  {
    ...
    difficulty: "Avanzado",
    prompt: "Muestra los coches cuyo precio está por encima del precio medio de todo el catálogo. Necesitas una subconsulta.",
    hint: "WHERE precio > (SELECT AVG(precio) FROM coches).",
    starterCode: STARTER,
    canonicalSQL: "SELECT * FROM coches WHERE precio > (SELECT AVG(precio) FROM coches) ORDER BY precio DESC;",
  ...
  },
  {
    ...
    difficulty: "Avanzado",
    prompt: "Cuenta cuántos coches hay en cada tramo: 'barato' (<25000), 'medio' (25000-50000), 'caro' (>50000). Devuelve tramo y total.",
    hint: "CASE WHEN ... THEN ... END + GROUP BY.",
    starterCode: STARTER,
    canonicalSQL: "SELECT CASE WHEN precio < 25000 THEN 'barato' WHEN precio <= 50000 THEN 'medio' ELSE 'caro' END AS tramo, COUNT(*) AS total FROM coches GROUP BY tramo ORDER BY tramo;",
  ...
  },
  {
    ...
    difficulty: "Avanzado",
    prompt: "Calcula el importe total vendido agrupado por la ciudad del cliente, ordenado descendentemente.",
    hint: "SUM(precio_venta) y un JOIN a clientes.",
    starterCode: STARTER,
    canonicalSQL: "SELECT cl.ciudad, SUM(v.precio_venta) AS total FROM ventas v JOIN clientes cl ON v.cliente_id = cl.id GROUP BY cl.ciudad ORDER BY total DESC;",
  ...
  },
  {
    ...
    difficulty: "Avanzado",
    prompt: "Devuelve la venta con el precio de venta más alto, mostrando modelo, cliente, fecha y precio.",
    hint: "ORDER BY precio_venta DESC LIMIT 1.",
    starterCode: STARTER,
    canonicalSQL: "SELECT c.modelo, cl.nombre AS cliente, v.fecha, v.precio_venta FROM ventas v JOIN coches c ON v.coche_id = c.id JOIN clientes cl ON v.cliente_id = cl.id ORDER BY v.precio_venta DESC LIMIT 1;",
...

  {
    ...
    difficulty: "Experto",
    prompt: "Para cada venta, muestra su cliente, la fecha, el precio de venta y el número de venta acumulado (ranking) de ese cliente, ordenado por fecha.",
    hint: "ROW_NUMBER() OVER (PARTITION BY cliente_id ORDER BY fecha).",
    starterCode: STARTER,
    canonicalSQL: "SELECT cl.nombre AS cliente, v.fecha, v.precio_venta, ROW_NUMBER() OVER (PARTITION BY v.cliente_id ORDER BY v.fecha) AS n_venta_cliente FROM ventas v JOIN clientes cl ON v.cliente_id = cl.id ORDER BY v.cliente_id, v.fecha;",
  ...
  },
  {
    ...
    difficulty: "Experto",
    prompt: "Calcula la media móvil de precio_venta de las últimas 3 ventas de cada cliente, ordenadas por fecha.",
    hint: "AVG(precio_venta) OVER (PARTITION BY cliente_id ORDER BY fecha ROWS 2 PRECEDING).",
    starterCode: STARTER,
    canonicalSQL: "SELECT cl.nombre AS cliente, v.fecha, v.precio_venta, ROUND(AVG(v.precio_venta) OVER (PARTITION BY v.cliente_id ORDER BY v.fecha ROWS 2 PRECEDING), 2) AS media_movil FROM ventas v JOIN clientes cl ON v.cliente_id = cl.id ORDER BY v.cliente_id, v.fecha;",
  ...
  },
  {
    ...
    difficulty: "Experto",
    prompt: "Calcula el porcentaje del total de ventas (en importe) que corresponde a cada marca. Devuelve marca, importe y porcentaje (2 decimales).",
    hint: "SUM ventas agrupado + dividir entre SUM total usando una subconsulta.",
    starterCode: STARTER,
    canonicalSQL: "SELECT m.nombre AS marca, SUM(v.precio_venta) AS importe, ROUND(100.0 * SUM(v.precio_venta) / (SELECT SUM(precio_venta) FROM ventas), 2) AS pct FROM marcas m JOIN coches c ON c.marca_id = m.id JOIN ventas v ON v.coche_id = c.id GROUP BY m.nombre ORDER BY pct DESC;",
  ...
  },
  {
    ...
    difficulty: "Experto",
    prompt: "Usando un self-join sobre coches, devuelve pares de modelos distintos de la misma marca (sin duplicados A-B / B-A), ordenados por marca y primer modelo.",
    hint: "JOIN coches c2 ON c1.marca_id = c2.marca_id WHERE c1.id < c2.id.",
    starterCode: STARTER,
    canonicalSQL: "SELECT m.nombre AS marca, c1.modelo AS modelo_a, c2.modelo AS modelo_b FROM coches c1 JOIN coches c2 ON c1.marca_id = c2.marca_id AND c1.id < c2.id JOIN marcas m ON c1.marca_id = m.id ORDER BY marca, modelo_a, modelo_b;",
